...
class Test_UpdateOrderViewmodel:
    def test_update_order_viewmodel(self):
        ...
        order = Order(id=1,pizza=Pizza(flavor=FLAVOR.BEEF,stuffed_edge=STUFFED_EDGE.RICOTTA), table=1)

        updateOrderViewmodel = UpdateOrderViewmodel(order = order).to_dict()

        assert updateOrderViewmodel == {"id": 1, "pizza":{"flavor":"BEEF","stuffed_edge":"RICOTTA"}, "message":"The order has been updated!"}